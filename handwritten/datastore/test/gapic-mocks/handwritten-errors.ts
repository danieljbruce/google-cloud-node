...
describe('HandwrittenLayerErrors', () => {
  describe('Can only specify one of transaction, consistency, readTime', () => {
    ...
    const testCases = [
      {
        options: {consistency: 'eventual', readTime: 77000} as RunQueryOptions,
        expectedError:
      ...
      {
        options: {consistency: 'eventual'} as RunQueryOptions,
        expectedError: 'Read consistency cannot be specified in a transaction.',
        description:
          'should error when new transaction and eventual consistency are specified',
      },
      {
        options: {readTime: 77000} as RunQueryOptions,
      ...
      describe(testParameters.description, () => {
        ...
        it('should error when runQueryStream is used', done => {
          ...
          try {
            ...
            expect(err.message).toBe(testParameters.expectedError);
            done();
          }
        });
